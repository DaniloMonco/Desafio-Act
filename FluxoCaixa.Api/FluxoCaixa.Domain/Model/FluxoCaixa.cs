...
{
    public record FluxoCaixa(int Ano, int Mes, IList<FluxoCaixaItem> Items)
    {
        //protected FluxoCaixa() : this(default, default, default) { }
        protected FluxoCaixa(int ano, int mes) : this(ano, mes, new List<FluxoCaixaItem>())
        {
        }

        public int Ano { get; protected set; } = Ano;
        public int Mes { get; protected set; } = Mes;

        public decimal Saldo => Items.Sum(i => i.Saldo);

        public IList<FluxoCaixaItem> Items { get; protected set; } = Items;
